...
async function generateHomeHero() {
    const filename = 'home-hero.png';
    const filepath = path.join(PUBLIC_IMAGE_DIR, filename);

    console.log(`🎨 Generating Home Hero Image (FLUX.1): ${filename}...`);

    const fullPrompt = `
    A high-quality, flat vector illustration of a futuristic digital workspace or dashboard.
    Style: Minimalist SaaS tech interface, deep navy (#002244) and white, clean lines.
    Elements: Floating UI cards, graphs, code snippets, automation nodes connecting (Zapier style).
    Composition: Wide shot, balanced, suitable for a hero background or side image.
    Vibe: Productivity, Automation, Intelligence, 'Building the Future'.
